...
const Home = () => {
  ...
  const handleSend = async (
    ...
    if (selectedConversation) {
      ...
      try {
        const response = await fetch('http://localhost:8000/api/generate_query_response/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          // body: JSON.stringify({ messages: updatedConversation.messages }),
          body: JSON.stringify({
            query: message.content,
            pdf_name: selectedFile?.name || null,
    ...
    const getPaperDetailsViaAPI = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:8000/api/get_research_paper_details/?pdf_url=${encodeURIComponent(pdfURL)}&filename=${encodeURIComponent(selectedFileName)}&user_id=${encodeURIComponent(userEmail)}`
        );
          ...
          <div className="flex h-full w-full pt-[48px] sm:pt-0">
            {showSidebar ? (
              <div>
                ...
                <button
                  className="fixed top-5 left-[350px] z-50 h-7 w-7 hover:text-gray-400 dark:text-white dark:hover:text-gray-300 sm:top-0.5 sm:left-[320px] sm:h-8 sm:w-8 sm:text-neutral-700"
            ...
            {showPromptbar ? (
              <div>
                ...
                />}
                <button
                  className="fixed top-5 right-[350px] z-50 h-7 w-7 hover:text-gray-400 dark:text-white dark:hover:text-gray-300 sm:top-0.5 sm:right-[350px] sm:h-8 sm:w-8 sm:text-neutral-700"
